...
// ✅ TASK STATS — must be before /tasks/:id routes to avoid conflict
app.get('/tasks/stats', auth, async (req, res) => {
  try {
    ...
    const urgent = tasks.filter(t => !t.completed && t.priority <= 2).length;
 
    res.json({ total, done, overdue, urgent, pending: total - done });
  } catch (err) {
...
function buildGraph(tasks) {
  let titleToId = {};
  tasks.forEach(task => {
    titleToId[task.title.toLowerCase()] = task._id.toString();
  });

  let graph = {};
  tasks.forEach(task => {
    graph[task._id.toString()] = (task.dependencies || [])
      .map(dep => titleToId[dep.toLowerCase()])
      .filter(Boolean);
  });
  return graph;
}
...
function topologicalSort(graph) {
  ...
  function dfs(node) {
    ...
    visited.add(node);
    for (let nei of graph[node] || []) {
      dfs(nei);  // visit dependencies FIRST
    }
    result.push(node);
  }

  // Visit nodes that have dependencies FIRST
  // so their dependencies get pushed before them
  Object.keys(graph).forEach(node => dfs(node));

  return result; // NO reverse!
}
